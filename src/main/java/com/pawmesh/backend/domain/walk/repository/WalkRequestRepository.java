...
public interface WalkRequestRepository extends JpaRepository<WalkRequest, Long> {
    ...
    List<WalkRequest> findByReceiverDog_PetIdAndStatus(Long receiverDogId, WalkRequestStatus status);

    /** 중복 요청 방지: 같은 상대에게 처리 대기중(PENDING)인 요청이 이미 있는지 */
    boolean existsByRequesterDog_PetIdAndReceiverDog_PetIdAndStatus(
